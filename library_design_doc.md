...
- **Implemented (Phase 5):** beds are **generated procedurally** (a Web Audio drone-chord + slow filter LFO + airy noise), one per culture plus a neutral atrium bed — owned/CC0, so nothing to source or license. Crossfade on travel, autoplay-gated on the enter gesture, mute via the 🔊 toggle / **M**. `RoomTheme.ambientAudioUrl` stays as the seam to swap in sourced field-recordings later without touching the engine.
